...
int main(int argc, char **argv) {
  ...
  switch (argc) {
    ...
    default:
      ...
      cerr << redSTR
          "Or: ./mono_tum path_to_vocabulary path_to_settings path_to_image_folder path_to_times_file "
          "path_to_odometryData (number of odometryData)"
           << endl;
      return 1;
  }
  ...
  VIEO_SLAM::System SLAM(argv[1], argv[2], VIEO_SLAM::System::MONOCULAR, true, map_sparse_name);
  g_pSLAM = &SLAM;  // zzh
  ...
  for (int ni = 0; ni < nImages; ni++) {
    ...
    SLAM.TrackStereo(vector<cv::Mat>{im}, tframe);
