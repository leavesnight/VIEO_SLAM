...
int main(int argc, char **argv){
	...
	ros::Subscriber sub_pts_and_pose = nodeHandler.subscribe("/kinect2_odom_publisher/pts_and_pose", 1000, ptCallback);
	ros::Subscriber sub_all_kf_and_pts = nodeHandler.subscribe("/kinect2_odom_publisher/all_kf_and_pts", 1000, loopClosingCallback);
...
void saveMap(unsigned int id) {
	...
	if (id > 0) {
		...
		cv::Mat grid_map_flip;
		cv::flip(grid_map_thresh,grid_map_flip,0);
		cv::imwrite(map_name_template + ".pgm", grid_map_flip);
	}
...
void processMapPt(const geometry_msgs::Point &curr_pt, cv::Mat &occupied, cv::Mat &visited, 
	...
	if (use_height_thresholding){
		...
		Eigen::Vector4d transformed_point_location = transform_mat * Eigen::Vector4d(pt_pos_x, pt_pos_y, pt_pos_z, 1);//from Xw to Xc
		double transformed_point_height = transformed_point_location[1] / transformed_point_location[3];//normalized y
		is_ground_pt = transformed_point_height < 0;//Height thresholding, here only accept points above XZ plane(pointing downside) as obstacles, you can change it 
	}
#ifndef DISABLE_FLANN
	if (use_plane_normals) {//Slope thresholding
	...
	//Bresenham Line-Drawing algorithm with integer version: https://www.cs.helsinki.fi/group/goa/mallinnus/lines/bresenh.html (though here is just float-version)
	...
	if (steep){//change the order of x&y to make it locate in right Bresenham area
		swap(x0, y0);
		swap(x1, y1);
	}
	if (x0 > x1){//make  x1>x0 && x1-x0>abs(y1-y0) to make it right to increase x
		swap(x0, x1);
		swap(y0, y1);
	}
	int dx = x1 - x0;
	int dy = abs(y1 - y0);
	double error = 0;//e here is ||e||
	double deltaerr = ((double)dy) / ((double)dx);//m=deltay/deltax, here is ||m|
	int y = y0;
	int ystep = (y0 < y1) ? 1 : -1;
	for (int x = x0; x <= x1; ++x){
		if (steep) {//for the order has been changed
			++visited.at<float>(x, y);
		}
		else {
			++visited.at<float>(y, x);
		}
		error = error + deltaerr;//e=e+m
		if (error >= 0.5){//if e>0: e+m>=0.5 else if e<0: e+m<=-0.5
			y = y + ystep;
			error = error - 1.0;//e=e+m-1
		}
	}
}
...
void processMapPts(const std::vector<geometry_msgs::Pose> &pts, unsigned int n_pts,
	...
	if (use_plane_normals) {//Slope thresholding
		...
		for (unsigned int pt_id = start_id; pt_id < end_id; ++pt_id){
			...
			flannResultT flann_result(results, 3, 1);//knn=3
			flann_index->knnSearch(flann_query, flann_result, flann_dists, 3, flann::SearchParams());//3 nearest points
			...
			Eigen::Matrix3d centered_pts = nearest_pts.colwise() - centroid;//rows are x,y,z; 3 cols correspond to 3 vectors from center to these 3 nearest points
			...
			Eigen::Vector3d normal_direction = svd.matrixU().col(n_cols - 1);//why???
	...
	if (use_local_counters) {
		...
		if (use_gaussian_counters) {//Gaussian smoothing of counters
...
void updateGridMap(const geometry_msgs::PoseArray::ConstPtr& pts_and_pose){
	...
	if (use_height_thresholding){//from Twc to Tcw!
	...
	unsigned int n_pts = pts_and_pose->poses.size() - 1;//pts_and_pose just comes kf's Pose firstly, then all local map points
...
void getGridMap() {
	...
	for (int row = 0; row < h; ++row){
		for (int col = 0; col < w; ++col){
			...
			if (visits <= visit_thresh){//Visit thresholding
	...
	if (use_boundary_detection) {//Canny boundary detection, need to investigate more!?
		cv::Mat canny_output;
		...
		for (int row = 0; row < h; ++row){
			for (int col = 0; col < w; ++col){
				if (canny_output.at<uchar>(row, col)>0) {
					grid_map_thresh.at<uchar>(row, col) = 0;
					grid_map_int.at<char>(row, col) = 100;//100% occupied
				}
			}
		}
...
void showGridMap(unsigned int id) {
	...
	if (show_camera_location) {
		...
		cv::Mat grid_map_flip;
		cv::flip(grid_map_rgb,grid_map_flip,0);
		cv::imshow("grid_map_thresh_resized", grid_map_flip);
// 		cv::imshow("grid_map_thresh_resized", grid_map_rgb);
	}
	else {
		cv::Mat grid_map_flip;
		cv::flip(grid_map_thresh_resized,grid_map_flip,0);
		cv::imshow("grid_map_thresh_resized", grid_map_flip);
