                ...
                !stopRequested())//if the newKFs list is idle and not requested stop by LoopClosing/localization mode
            {
                ...
                if (mpMap->KeyFramesInMap() >
                    ...
                    if (!mpIMUInitiator->GetVINSInited()) {
                        ...
                        Optimizer::LocalBundleAdjustmentNavStatePRV(mpCurrentKeyFrame, mnLocalWindowSize, &mbAbortBA,
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    if (mpCurrentKeyFrame->getState()==(char)Tracking::ODOMOK) {//added by zzh, it can also be put in InsertKeyFrame()
        if (mnLastOdomKFId > 0 && mpCurrentKeyFrame->mnId <= mnLastOdomKFId +
                                                             5) {//5 is the threshold of Reset() soon after initilization in Tracking, here we will clean these middle state==OK KFs for a better map
            //one kind of Reset()
