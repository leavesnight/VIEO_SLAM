...
int EncPreIntegrator::PreIntegration(const double &timeStampi, const double &timeStampj,
                                     const listeig(EncData)::const_iterator &iterBegin,
                                     const listeig(EncData)::const_iterator &iterEnd, bool breset) {
  if (iterBegin != iterEnd) {  // timeStampi may >=timeStampj for Map Reuse
    if (breset) reset();
    const double EPS = 1E-5;

    //     listeig(EncData)::const_iterator it=iterEnd;
    //     std::cout<<timeStampi<<" "<<timeStampj<<" "<<timeStampi-iterBegin->mtm<<"
    //     "<<timeStampj-(--it)->mtm<<std::endl;
    ...
    for (listeig(EncData)::const_iterator iterj = iter_start;
         iterj != iter_stop;) {                          // start iterative method from i/iteri->tm to j/iter->tm
      ...
      if (deltat > 1.5) {
        mdeltatij = 0;
        cout << redSTR "Check Odometry!" << whiteSTR << endl;
        return -1;
      }  // this filter is for the problem of my dataset which only contains encoder data

      // select/design measurement_j-1
      double vl, vr;  // vlj-1,vrj-1
      vl = iterjm1->mv[0];
      vr = iterjm1->mv[1];  // this way seems to be more precise than the following one (arithmatical average) for
                            // Corridor004
      /*
      if (iterj!=iter_stop){
...
// 	}
      ...
      // calculate Sigmaij firstly to use deltaThetaijMz as deltaTheta~ij-1z, maybe we can use deltaP~ij-1 instead of
      // vf/w here
      double thetaj_1j = w * deltat;  // Theta~ej-1ej
      Matrix6d A(Matrix6d::Identity()), C;
      Matrix<double, 6, 2> B;
      Matrix3d Rj_1j;  // delta~REj-1Ej
      Rj_1j << cos(thetaj_1j), -sin(thetaj_1j), 0, sin(thetaj_1j), cos(thetaj_1j), 0, 0, 0, 1;
      Matrix3d Rij_1;
      Rij_1 << cos(deltaThetaijMz), -sin(deltaThetaijMz), 0, sin(deltaThetaijMz), cos(deltaThetaijMz), 0, 0, 0, 1;
      ...
      if (abs(thetaj_1j) < EPS) {
        ...
        sinthdivw = deltat;
        one_costh_divw = w * dt2 / 2;
        Bx = -vf * w * dt2 * deltat / 2;
        By = vf * dt2 / 2;
        C0 = 0;
        C1 = -vf * dt2 / 2;
      } else {
        sinthdivw = sin(thetaj_1j) / w;
        one_costh_divw = (1 - cos(thetaj_1j)) / w;
        Bx = vf / w * (deltat * cos(thetaj_1j) - sinthdivw);
        By = vf / w * (deltat * sin(thetaj_1j) - one_costh_divw);
        C0 = vf / w * (deltat - sinthdivw);
        C1 = -vf / w * one_costh_divw;
      }
      A.block<3, 3>(3, 0) = Rij_1 * Sophus::SO3exd::hat(Vector3d(-vf * sinthdivw, -vf * one_costh_divw, 0));
      Bj_11.block<3, 2>(0, 0) << sinthdivw / 2 - Bx / 2 / rc, sinthdivw / 2 + Bx / 2 / rc,
          one_costh_divw / 2 - By / 2 / rc, one_costh_divw / 2 + By / 2 / rc, 0, 0;
      C(0, 3) = sinthdivw;
      C(0, 4) = one_costh_divw;
      C(1, 3) = -one_costh_divw;
      C(1, 4) = sinthdivw;
      Cj_11 << sinthdivw, -one_costh_divw, 0, 0, 0, Bx, one_costh_divw, sinthdivw, 0, 0, 0, By, 0, 0, deltat, C0, C1, 0;
      B.block<3, 2>(3, 0) = Rij_1 * Bj_11;
      C.block<3, 6>(3, 0) = Rij_1 * Cj_11;
      if (EncData::mdt_cov_noise_fixed)  // eta->etad
        mSigmaEij =
            A * mSigmaEij * A.transpose() + B * eigSigmaeta * B.transpose() + C * eigSigmaetam * deltat * C.transpose();
      else if (!EncData::mFreqRef || deltat < 1.5 / EncData::mFreqRef)
        mSigmaEij = A * mSigmaEij * A.transpose() + B * (eigSigmaeta / deltat) * B.transpose() +
                    C * (eigSigmaetam * deltat) * C.transpose();
      else
        mSigmaEij = A * mSigmaEij * A.transpose() + B * (eigSigmaeta * EncData::mFreqRef) * B.transpose() +
                    C * (eigSigmaetam * deltat) * C.transpose();
      ...
      if (abs(thetaj_1j) < EPS) {                   // or thetaj_1j==0
        // double
        // arrdTmp[4]={cos(deltaThetaijMz),sin(deltaThetaijMz),-sin(deltaThetaijMz),cos(deltaThetaijMz)};//row-major:{cos(deltaTheij-1Mz),-sin(deltaTheij-1Mz),0,sin(deltaTheij-1Mz),cos(deltaThetaij-1Mz),0,0,0,1};
        // but Eigen defaultly uses col-major!
        // eigdeltaPijM+=Matrix2d(arrdTmp)*Vector2d(vf*deltat,0);//deltaPijM+Reiej-1*vej-1ej-1*deltat
        eigdeltaPijM += Rij_1.block<2, 2>(0, 0) *
                        Vector2d(vf * sinthdivw, vf * one_costh_divw);  // deltaPijM+Reiej-1*vej-1ej-1*deltat
      } else {
        eigdeltaPijM += vf / w * Vector2d(sin(thetaij) - sin(deltaThetaijMz), cos(deltaThetaijMz) - cos(thetaij));
      }
      // update deltaThetaijM
      deltaThetaijMz =
          thetaij;  // deltaThetaij-1M + weiej-1 * deltatj-1j, notice we may use o in the code instead of e in the paper
      // mdeltatij += deltat;
    }
