...
int IMUInitialization::reduceKFs(const vector<char> &reduced_hids, vector<vector<IMUKeyFrameInit *> *> &vKFsInit,
...
bool IMUInitialization::TryInitVIO_zzh(
  ...
  for (int i = 0; i < num_handlers; ++i) {
    ...
    if (verbose) {
      ...
      ++mean_times;
      PRINT_INFO_MUTEX("OptimizeInitialGyroBias cost : " << duration << "s; max = " << max_duration
                                                         << "; mean = " << tot_duration / mean_times << endl);
    }
  }
  // reduce vKFsInit to vKFsInit2
  vector<char> id_cams, id_cams2;
  //        cerr<<"check reduce bef:"<<vKFsInit.size()<<" "<<vKFsInit[0]->size()<<"
  //        "<<(*vKFsInit[0])[1]->mOdomPreIntIMU.getlOdom().size()<<endl; for (int i = 0; i < (*vKFsInit[0]).size();
  //        ++i)
  ...
  //            cerr<<" "<<SO3exd((*vKFsInit2[0])[i]->mOdomPreIntIMU.mRij).log().transpose()<< ", t=" <<
  //            (*vKFsInit2[0])[i]->mOdomPreIntIMU.mpij.transpose()<<"; ";
  //        cerr<<endl;
  ...
  for (int h = 0, last_h = h; h < num_handlers; ++h) {
    ...
    for (int i = 0, last_i = i; i < Ns[h] - 2; ++i) {
      ...
      Vector3calc gamma = (Rc1 * pcb[0] - Rc2 * pcb[1]) * dt23 + (Rc3 * pcb[2] - Rc2 * pcb[1]) * dt12 -
                          Rc2 * (Rcb[1] * dp23) * dt12 - Rc1 * (Rcb[0] * dv12) * dt12 * dt23 +
                          Rc1 * (Rcb[0] * dp12) * dt23;
  ...
  if (!tot_imu_init_enable &&
      3 * num_eq < num_var_opt) {  // for more robust judgement instead of judging KeyFramesInMap()
  ...
  for (int h = 0; h < num_handlers; ++h) {
    ...
    for (int i = 0; i < Ns[h] - 2; ++i) {
      ...
      const Matrix3calc &Rcb1 = Rcbs_h[reduced_i].matrix(), &Rcb2 = Rcbs_h[reduced_i + 1].matrix(),
                        &Rcb3 = Rcbs_h[reduced_i + 2].matrix();
      const Vector3calc &pcb1 = pcbs_h[reduced_i], &pcb2 = pcbs_h[reduced_i + 1], &pcb3 = pcbs_h[reduced_i + 2];
      ...
      Vector3calc psi = (Rc1 * pcb1 - Rc2 * pcb2) * dt23 + (Rc3 * pcb3 - Rc2 * pcb2) * dt12 -
                        Rc2 * (Rcb2 * dp23) * dt12 - Rc1 * (Rcb1 * dv12) * (dt12 * dt23) + Rc1 * (Rcb1 * dp12) * dt23 -
                        (dt12 * dt12 * dt23 + dt12 * dt23 * dt23) / 2 * (RwI * GI);
  ...
  if (6 == num_var_opt2) s_ = y(0);                     // s*_C, C means IV-C in the paper
    ...
    {  // Update the Map needs mutex lock: Stop local mapping, like RunGlobalBundleAdjustment() in LoopClosing.cc
      ...
      for (vector<KeyFrame *>::const_iterator vit = vScaleGravityKF.begin(), vend = vScaleGravityKF.end(); vit != vend;
        ...
        if (pKF != vScaleGravityKF.back()) {
          ...
          Vector3calc vwbi =
              -1. / dt * (scale * (wPc - wPcnext) + (Rwc * pcb - Rwcnext * pcb) + dt * dt / 2 * gw + Rwc * Rcb * (dp));
