...
class TemplatedVocabulary {
 ...
 protected:
  ...
  std::vector<Node *>
      m_words;  // maybe recording world_id instead of &m_nodes[leafid]/leafid in BowVector is to make id small
};
...
void TemplatedVocabulary<TDescriptor, F>::transform(const std::vector<TDescriptor> &features, BowVector &v,
  ...
  if (m_weighting == TF || m_weighting == TF_IDF) {
    ...
    for (fit = features.begin(); fit < features.end(); ++fit, ++i_feature) {
      ...
      WordValue w;
      // w is the idf value if TF_IDF, 1 if TF
      transform(*fit, id, w, &nid, levelsup);  // rectify id,w by searching the leaf node infomation && nid(&nid!=NULL)
                                               // by recording the leaf's parent id at the level (d-levelsup)

      if (w > 0)  // not stopped
      {
        v.addWeight(id, w);             // get v[id]=ni*IDF
        fv.addFeature(nid, i_feature);  // recording some words' level(d-levelsup) parent node id's corresponding
                                        // feature indices of a certain Frame
      }
    }
...
void TemplatedVocabulary<TDescriptor, F>::transform(const TDescriptor &feature, WordId &word_id, WordValue &weight,
  ...
  do {
    ...
    double best_d =
        F::distance(feature, m_nodes[final_id].descriptor);  // calculate the hamming distance by the fastest way

    for (nit = nodes.begin() + 1; nit != nodes.end();
         ++nit)  // find the min distance in its children; for nodes[0] has already been calculated in best_d
    {
      NodeId id = *nit;
...
bool TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(const std::string &filename) {
  ...
  // nodes
  int expected_nodes = (int)((pow((double)m_k, (double)m_L + 1) - 1) / (m_k - 1));  // 1+k+k^2+k^3+...+k^d d==L
...
bool TemplatedVocabulary<TDescriptor, F>::loadFromBinaryFile(const std::string &filename) {
  ...
  if (m_k < 0 || m_k > 20 || m_L < 1 || m_L > 10 || n1 < 0 || n1 > 5 || n2 < 0 || n2 > 3 ||
      size_node != (unsigned int)(F::L + 9)  // should be 4(int)+32+4(float)+1(bool)
      || nb_nodes > (pow((double)m_k, (double)m_L + 1) - 1) /
                        (m_k - 1))  // 1+k+k^2+k^3+...+k^d d==L, it should be>= nb_nodes+1
  {
...
void TemplatedVocabulary<TDescriptor, F>::saveToBinaryFile(const std::string &filename) const {
  ...
  for (size_t i = 1; i < nb_nodes; i++) {
    ...
    _weight = node.weight;
    f.write((char *)&_weight, sizeof(_weight));  // double to float
